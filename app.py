...
def fetch_stock_data(ticker):
    """Fetches historical stock data for the last 3 years."""
...
def fetch_news(ticker):
    """Scrapes real-time news from Yahoo Finance."""
...
def fetch_random_news():
    ...
    return random.sample(random_news, min(5, len(random_news)))
...
def calculate_risk(data, ticker):
    """Calculates stock volatility and fetches Beta."""
    volatility = data['Close'].std()
    beta_info = fetch_stock_info(ticker)[3]
    # Handle case where beta is 'N/A'
    beta = beta_info if isinstance(beta_info, (int, float)) else 1.0
    return volatility, beta

def calculate_risk_score(volatility):
    """Determines a risk score based on volatility."""
    if volatility < 0.1:
        return "Low"
    elif volatility < 0.3:
        return "Moderate"
...
def calculate_stability_score(beta):
    """Determines a stability score based on Beta."""
...
def sentiment_analysis(current_price, predicted_price):
    """Generates a sentiment recommendation based on predicted vs current price."""
    if isinstance(current_price, str) or current_price == 0.0:
        return {
            "recommendation": "Hold 🟠",
            "comment": "Data unavailable for precise recommendation.",
            "color": "orange",
            "positive": 33, "neutral": 34, "negative": 33
        }

    if predicted_price > current_price * 1.01: # 1% threshold for Buy
        return {
            "recommendation": "Buy 🚀",
            "comment": "The stock is expected to rise significantly! A great time to invest.",
            "color": "green",
            "positive": 70,
            "neutral": 20,
            "negative": 10
        }
    elif predicted_price < current_price * 0.99: # 1% threshold for Sell
        return {
            "recommendation": "Sell 🔴",
            "comment": "The stock is expected to drop significantly. Consider selling.",
            "color": "red",
            "positive": 10,
            "neutral": 20,
            "negative": 70
...
def fetch_insider_trading(ticker):
    """Fetches top 5 insider transactions."""
    stock = yf.Ticker(ticker)
    try:
        insider = stock.insider_transactions
        if insider is not None and not insider.empty:
            return insider.head(5)
    except:
        pass
    return None

def fetch_sector_performance():
    """Fetches 1-month performance for major sector ETFs."""
    sectors = {
...
def compare_stocks(tickers):
    """Compares key metrics for a list of stocks."""
    comparison_data = []
    for ticker in tickers:
        ...
        if not data.empty and len(data) > 1:
            try:
                growth = (data["Close"][-1] - data["Close"][0]) / data["Close"][0] * 100
            except:
        ...
        comparison_data.append({
            ...
            "Volume": info.get('regularMarketVolume', 'N/A'),
            "Beta": info.get('beta', 'N/A'),
            "3Y Growth (%)": f"{growth:.2f}" if isinstance(growth, float) else growth
        })
    return pd.DataFrame(comparison_data)

def financial_health_check(ticker):
    """Provides key financial metrics for a stock's health."""
    stock = yf.Ticker(ticker)
    info = stock.info
    
    profitability = info.get('profitMargins', 0) * 100
    debt_levels = info.get('debtToEquity', 0)
    cash_flow = info.get('operatingCashflow', 0)
    roe = info.get('returnOnEquity', 0) * 100
    
    # Handle None/NaN values by setting to 0 for calculation
    profitability = profitability if profitability else 0
    debt_levels = debt_levels if debt_levels else 0
    cash_flow = cash_flow if cash_flow else 0
    roe = roe if roe else 0

    # Calculate Health Score (1-10) - Simplified Heuristic
    health_score = (
        (profitability / 20) +
        (10 - min(debt_levels / 100, 10)) + # D/E over 100% is penalized
        (cash_flow / 1e9) +
        (roe / 20)
    )
    # Normalize score
    health_score = min(max(health_score / 2.5, 1), 10)

    return {
        "Profitability 🏦": f"{profitability:.2f}%",
        "Debt/Equity Ratio ⚖️": f"{debt_levels:.2f}",
        "Operating Cash Flow 💵": f"${cash_flow / 1e9:.2f}B",
...
def predict_stock_prices(data, days, model_type):
    """Predicts stock prices using the selected model (ARIMA or LSTM)."""
    if data.empty:
    ...
    if model_type == "ARIMA":
        y_arima = data['Close'].values
        
        try:
            model = ARIMA(y_arima, order=(5, 1, 0)) # Standard ARIMA parameters
            model_fit = model.fit()
        ...
        if len(scaled_data) <= time_step + 1:
            st.error("Not enough data to train LSTM (Need more than 61 days).")
            return [], []
        ...
        preds = []
        curr_input = last_60_days.reshape(1, time_step, 1)
        
        for _ in range(days):
            pred_price = model.predict(curr_input, verbose=0)
            preds.append(pred_price[0][0])
            # Update input for the next prediction
            curr_input = np.append(curr_input[:, 1:, :], pred_price.reshape(1, 1, 1), axis=1)
...
def generate_graph(data, chart_type, title_suffix=""):
    """Generates historical stock price graph."""
    fig = go.Figure()
...
def generate_prediction_graph(future_dates, predictions, chart_type):
    """Generates predicted stock price graph."""
    fig = go.Figure()

    # NOTE: Candlestick/OHLC for predictions is highly unconventional as we only predict
    # the closing price. For consistency, we'll plot a simple line/bar chart.
    
    if chart_type in ["Line Chart", "Candlestick", "OHLC"]:
        fig.add_trace(go.Scatter(
            x=future_dates, 
            y=predictions, 
            mode='lines+markers',
...
def generate_combined_graph(data, future_dates, predictions, chart_type):
    """Generates a combined historical and predicted price graph."""
    fig = go.Figure()

    # Historical Data (Handles all chart types)
    if chart_type == "Line Chart":
        fig.add_trace(go.Scatter(
            x=data.index, y=data["Close"], mode='lines', name="Historical Price",
            line=dict(color='royalblue', width=2)
        ))
    elif chart_type == "Candlestick":
        fig.add_trace(go.Candlestick(
            x=data.index, open=data['Open'], high=data['High'], low=data['Low'],
            close=data['Close'], name="Historical Candlestick"
        ))
    elif chart_type == "OHLC":
        fig.add_trace(go.Ohlc(
            x=data.index, open=data['Open'], high=data['High'], low=data['Low'],
            close=data['Close'], name="Historical OHLC"
        ))
    elif chart_type == "Bar Chart":
        fig.add_trace(go.Bar(
            x=data.index, y=data["Close"], name="Historical Price"
        ))

    # Predicted Data (Only line plot is practical for prediction)
    fig.add_trace(go.Scatter(
        x=future_dates, y=predictions, mode='lines+markers', name="Predicted Price",
        line=dict(color='orange', width=2, dash='dot'),
        marker=dict(size=8, color='red', symbol='circle-open')
    ))
...
def generate_sentiment_pie_chart(sentiment):
    """Generates a pie chart for sentiment scores."""
    labels = ["Positive", "Neutral", "Negative"]
    values = [sentiment["positive"], sentiment["neutral"], sentiment["negative"]]
    colors = ["#87CEEB", "#1E3A8A","#FFB6C1"] 
    ...
    fig.update_layout(
        ...
        showlegend=False,
        annotations=[dict(text=sentiment["recommendation"].split(' ')[0], x=0.5, y=0.5, font_size=20, showarrow=False)]
    )
...
def manual_currency_converter(amount, from_currency, to_currency):
    """Converts currency using manually defined rates."""
    conversion_rates = {
        "USD": 1.0, "EUR": 0.85, "GBP": 0.73, "JPY": 110.0,
        "INR": 87.15, "AUD": 1.35, "CAD": 1.25, "CHF": 0.92,
        "CNY": 6.45, "SEK": 8.65, "NZD": 1.45,
    }
...
def main():
    ...
    with st.sidebar:
        ...
        model_type = st.selectbox("Select Prediction Model", ["ARIMA", "LSTM"])
        ...
        currency_options = [
            "USD (United States Dollar)", "EUR (Euro - European Union)", 
            "GBP (British Pound Sterling - United Kingdom)", "JPY (Japanese Yen - Japan)", 
            "INR (Indian Rupee - India)", "AUD (Australian Dollar - Australia)", 
            "CAD (Canadian Dollar - Canada)", "CHF (Swiss Franc - Switzerland)", 
            "CNY (Chinese Yuan - China)", "SEK (Swedish Krona - Sweden)", 
            "NZD (New Zealand Dollar - New Zealand)"
        ]
        
        from_currency = st.selectbox("From Currency", currency_options, index=0)
        to_currency = st.selectbox("To Currency", currency_options, index=4)
    ...
    if st.session_state.predict_clicked:
        try:
            ...
            if data.empty:
                ...
                st.session_state.predict_clicked = False # Reset click state
            else:
                # Prediction with a spinner
                with st.spinner(f"Running {model_type} prediction... This may take a moment."):
                ...
                if len(predictions) > 0:
                    ...
                    sentiment = sentiment_analysis(current_price_val, predicted_price)

                    # --- Header ---
                    st.markdown(f"""
                        <div class="fade-in">
                            <h2 style="text-align: center;">{company_name} ({ticker}) - Prediction Results 📊</h2>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    st.subheader("🎯 Recommendation & Sentiment")
                    col_rec, col_pie = st.columns([2, 1])
                    ...
                    with col_rec:
                        st.markdown(f"""
                            <div style="background-color: {sentiment['color']}; padding: 20px; border-radius: 15px; text-align: center;">
                                ...
                                <p style="font-size: 18px;">**{sentiment['comment']}**</p>
                                <p>Predicted Price for {future_dates[0].date()}: **${predictions[0]:.2f}**</p>
                            </div>
                        """, unsafe_allow_html=True)
                    ...
                    # --- Risk Analysis ---
                    st.subheader("📊 Risk and Stability Analysis")
                    volatility, beta = calculate_risk(data, ticker)
                    risk_score = calculate_risk_score(volatility)
                    stability_score = calculate_stability_score(beta)
                    beta_disp = f"{beta:.2f}" if isinstance(beta, (int, float)) else "N/A"
                    ...
                    with risk_col2:
                        st.markdown(f"""
                            <div class="glass-card fade-in">
                                ...
                                <div class="metric-value">{beta_disp}</div>
                            </div>
                        """, unsafe_allow_html=True)
                    with risk_col3:
                        st.markdown(f"""
                    ...
                    with risk_col4:
                        st.markdown(f"""
                            <div class="glass-card fade-in">
                                <div class="metric-title">🛡️ Stability</div>
                    ...
                    # Prediction Graph
                    st.markdown(f"### {model_type} Model Prediction")
                    ...
                    st.dataframe(data[['Open', 'High', 'Low', 'Close', 'Volume']].tail(5).style.format({
                    ...
                    st.subheader("❤️ Financial Health Check")
                    ...
                    st.subheader("🌎 Sector-Wise Performance (Last 1 Month)")
                    sector_performance = fetch_sector_performance()
                    if sector_performance:
                        df_sector = pd.DataFrame.from_dict(sector_performance, orient="index", columns=["Performance (%)"])
                        st.dataframe(df_sector.style.bar(subset=['Performance (%)'], align='zero', color=['#d65f5f', '#5fba7d']).format({
                            'Performance (%)': '{:.2f}%'
                        }))
                    else:
                    ...
                    # Historical Market Insights & Fun Facts
                    st.subheader("📜 Did You Know? (Market History)")
                    st.markdown(f"""
                        <div class="glass-card fade-in">
